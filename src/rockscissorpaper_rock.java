...
public class rockscissorpaper_rock extends JFrame {
	...
	private Image introbackground;
	  Toolkit kit = Toolkit.getDefaultToolkit();
		int cnt=0;
	public rockscissorpaper_rock(int num) {
		...
		setResizable(false);
		String[] papercw = {"select2_rock(wrong)1.png" , "select2_rock(correct)1.png", "select2_rock(correct)2.png"};
		...
		if(num==0) {panel.add(again);panel.add(next);}
		if(num==1) {panel.add(next);}
		next.addActionListener(new ActionListener() {//okButton을 누르면
	        public void actionPerformed(ActionEvent e) {
	        	cnt++;
	        	if(num==0) {new wrong1_2(3);dispose();}
	        	if(cnt==3) {
	        		new gameStory5();dispose();
	        	}
	        	else {
	        	startScreen = new ImageIcon("images/backgrounds/"+papercw[num+1]);
	        	}
			}
		});
